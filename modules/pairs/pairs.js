...
const createPair = async (req, res) => {
    ...
    const query = JSON.stringify({
        ...
        response = JSON.parse(await DBase.create(`${DBPrefix}bot_pair`, query)),
...
const getPairById = async (req, res) => {
    const query = JSON.stringify({ filter: req.params }),
        response = await DBase.read(`${DBPrefix}bot_pair`, query)
    ...
    if (response.records && response.records.length > 0) {
        ...
        if (pairBotId) {
            ...
            const botResponse = await DBase.read(`${DBPrefix}bot_bot`, queryBot);
...
const getPairs = async (req, res) => {
    ...
    if (requestExchangeId && requestExchangeId >= 0) {
        ...
        const botsByExchangeId = await DBase.read(`${DBPrefix}bot_bot`, JSON.stringify(botsQuery)),
    ...
    const promises = records.map(async (record) => {
        ...
        const ordersOpenedResponse = await DBase.read(`${DBPrefix}bot_grid`, queryOrdersOpened);
        ...
        const inTradesResponse = await DBase.read(`${DBPrefix}bot_grid`, inTradesQuery);
        ...
        const profitResponse = await DBase.read(`${DBPrefix}bot_grid`, queryProfit);
...
const updatePairById = async (req, res) => {
    ...
    const checkPairQuery = JSON.stringify({
        ...
        checkPairResponse = await DBase.read(`${DBPrefix}bot_pair`, checkPairQuery),
        pairToUpd = checkPairResponse.records[0]

    if (pairState !== pairToUpd.state) {
        if (pairState === 1 || pairState === 2) {
            const setPauseStartEndResponse = await helper.setPauseStartEnd('pair', pairId, updQuery.state, false)
    ...
    const response = await DBase.update(`${DBPrefix}bot_pair`, query)
